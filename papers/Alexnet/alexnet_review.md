...
이전까지는 딥러닝이 쓸모가 없다고 여겨졌다. 레이어를 깊게 쌓이면 오히려 학습 효율이 떨어졌다. Alexnet의 등장으로 비로소 머신러닝보다 딥러닝이 효율이 좋다는 것이 알려졌다.
CNN에서 딥러닝으로 AI 학습의 트렌드가 넘어올 수 있었던 이유가 AlexNet의 알고리즘이다. 깊은 신경망에서 학습하는 방법을 처음 제시한 논문이기 때문에 첫 논문 스터디의 논문으로 선택하였다.
